<script lang="ts">
  ...
  const headerLabel = $derived(
    statusFilter === 'delivered' ? 'Pagado' : 'Pendiente'
  );
...
<div class="min-h-dvh bg-zinc-50 text-zinc-900">
  ...
  <main class="mx-auto max-w-lg px-4 pb-24 pt-4 space-y-6">
    <div class="relative">
      <input
    ...
    {#if status.debtors.length === 0}
      <section class="text-center py-16 text-zinc-600">
        ...
        <button
          class="mt-4 inline-flex items-center justify-center rounded-lg px-4 py-2 text-sm font-medium bg-zinc-900 text-white hover:bg-zinc-800"
            ...
            <div class="flex items-end justify-between">
              ...
              <span class="text-sm text-zinc-600"
                ...
                <span class="font-medium"
                  >{currency(filteredSumForDebtor(debtor.id))}</span
                ></span
              >
            </div>
